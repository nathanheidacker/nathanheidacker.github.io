...
function createGrass(
    ...
    const vertices = new Float32Array([
        0.0, 1.0, 0.0, -0.2, -0.2, 0.0, 0.2, -0.2, 0.0,
    ]);
    ...
    geometry.setAttribute("height", new THREE.InstancedBufferAttribute(instanceHeights, 1))
    ...
    const material = new THREE.ShaderMaterial({
        ...
        vertexShader: /*glsl*/ `
            ...
            void main() {
                ...
                float randomLean = random(vWorldPos);
                float curveAmount = randomLean * (height / 3.0);
    ...
    for (let i = 0; i < count; i++) {
        dummy.position.set(Math.random() * width, 0, Math.random() * depth);
...
function Environment() {
    ...
    useEffect(() => {
        if (container.current) {
            ...
            const grass = createGrass(10, 10, 1000);
            grass.position.set(-20, 0, -20);
            scene.add(grass);
            ...
            const animate = () => {
                for (let block of [block1, block2]) {
